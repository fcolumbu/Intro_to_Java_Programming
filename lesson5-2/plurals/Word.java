...
public class Word
{
    ...
    */
    public String getPluralForm()
    {
        ...
        int len = this.letters.length();
        if (letters.substring(len-1, len).equals("y") &&
            isConsonant(len-2))
            {
                return(this.letters = letters.substring(0, len-1) + "ies");
            }
        // If the word ends in y preceded by a vowel, you just add an s.
        else if (letters.substring(len-1, len).equals("y") &&
            isVowel(len-2))
            {
                return(this.letters = letters.substring(0, len) + "s");
            }
        // You add an es when a word ends in o, or s, or sh, or ch.
         else if (letters.substring(len-1, len).equals("o") ||
                  letters.substring(len-1, len).equals("s") ||
                  letters.substring(len-2, len).equals("sh") ||
                  letters.substring(len-2, len).equals("ch"))     
            {
                return(this.letters = letters.substring(0, len) + "es");
            }       
